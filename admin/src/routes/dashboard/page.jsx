...
const DashboardPage = () => {
    ...
    const [topEstates, setTopEstates] = useState([]);
    ...
    return (
        <div className="flex flex-col gap-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                <div className="card">
                    <div className="card-header">
                        ...
                        <p className="card-title">Total Estates</p>
                    </div>
                    <div className="card-body bg-slate-100 transition-colors dark:bg-slate-950">
                        {loading ? (
                            ...
                            <>
                                <p className="text-3xl font-bold text-slate-900 transition-colors dark:text-slate-50">{estateCount}</p>
                                <div className="flex items-center gap-x-2">
                                    <TrendingUp size={18} />
                                    <p className="text-sm font-medium text-blue-500">25%</p>
                                </div>
                            </>
                        )}
                    </div>
            ...
            <div className="card">
                <div className="card-header">
                    <h2 className="text-xl font-semibold">Top Estates</h2>
                </div>
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b">
                                        ...
                                        <th className="py-2 text-left">Estate</th>
                                        <th className="py-2 text-left">Price</th>
                                        <th className="py-2 text-left">Status</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {topEstates.map((estate, index) => (
                                        <tr key={estate.id} className="border-b">
                                            ...
                                            <td className="py-2">
                                                <div className="flex items-center gap-x-3">
                                                    ...
                                                    />
                                                    <div>
                                                        <p className="font-medium">{estate.title}</p>
                                                        <p className="text-sm text-gray-500">{estate.description?.substring(0, 50)}...</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-2">${parseFloat(estate.price).toLocaleString()}</td>
                                            <td className="py-2">
